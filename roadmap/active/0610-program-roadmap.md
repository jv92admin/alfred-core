...
**Status:** Not Started (Track A item A0 ✅ done 2026-06-10 — seam contract agreed)
...
| A1 | **Protocol split (C-1)** | `DomainContext` + `AgentConfig` protocols; `DomainConfig` = composed (Kitchen/FPL untouched, zero migration); **`pre_write` lands in `DomainContext`** (amendment of record); CI import-linter: `alfred.context` imports no langgraph/instructor | Not Started |
...
| X2 | **Doc debt** | core-public-api.md stale "4 wheel targets" fixed; P5 kitchen-residue sweep (`"quick weeknight meals"`, `["milk","eggs"]`, `__init__.py` Pantry docstring, crud.py "ingredient enrichment") | Not Started |
