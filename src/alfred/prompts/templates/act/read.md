...
Count, sum, or average instead of fetching rows. Use when the user asks "how many", "total", "average" — not when they want to see the actual records.
...
**Rules:**
- Aggregate results are scalars — no IDs, no refs, can't be referenced in later steps
- `columns`, `limit`, `order_by` are ignored when `aggregate` is set
- Filters work normally (WHERE before aggregation)
