...
`post_read()` fires after the database query returns, before results are passed back to the pipeline. Use it to enrich records (e.g., translate integer FK IDs to names) or filter results that can't be filtered at the query level. Default: pass-through (returns records unchanged).
...
```
execute_crud()
    ├── db_read:   middleware.pre_read()  → modify query → execute → middleware.post_read() → return
...
| Read post-processing | Calls `middleware.post_read()` after query | `CRUDMiddleware.post_read()` — record enrichment, computed fields |
