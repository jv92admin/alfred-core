...
The Act node is unique — it has 6 template files that get composed into a single system prompt. The assembly order depends on the step's execution pattern:
...
The Act node has the most complex prompt assembly because it handles 4 execution patterns with different requirements and loops within a step.
...
The final prompt is joined with `\n\n` separators. Sections are ordered for optimal LLM attention. Each pattern determines which of these sections are relevant:
