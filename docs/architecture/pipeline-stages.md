...
**Execution patterns** — each `step_type` defines an I/O pattern that controls what tools Act loads, what context it provides, and which model tier it uses (`state.py:96`):
...
Act is the **execution engine**. It executes Think's plan step-by-step, dispatching each step according to its execution pattern.

### Pattern-Based Dispatch

Act loads different tools, prompts, and context based on the step's `step_type`:

| Pattern | Tools Available | System Prompt | Context Loaded | Output |
|---------|----------------|---------------|----------------|--------|
...
| `analyze` | `db_analyze`, `calculate` + domain-configurable | `base.md` + `crud.md` + `analyze.md` | Schema, prior step results, profile, guidance | Tool output (aggregates, arithmetic) + LLM reasoning |
